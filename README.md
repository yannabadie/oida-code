...
**Phase 3.5 + E1 + E2 + E3 complete — structural measurement pipeline
...
variant comparison / real-repo shadow smoke, and E3 estimator contracts
...
tests_pass/operator_accept, LLM input/output contracts (no LLM
called), and `assess_estimator_readiness` ladder.
...
fixture proves shadow pressure now varies with evidence;
**332/335 unit tests green (3 skips = V2 placeholder + 2 Phase-4
...
and produces `status="blocked"` on real repos at v0.4.x — the
correct state until LLM estimators arrive.

**Phase 4.0 LLM estimator dry-run pending.**
