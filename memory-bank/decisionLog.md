...
[2026-04-29 14:00:00] - **ADR-64: Methodology consolidation post-Phase-6.2 — `docs/project_status.md` rewrite, `BACKLOG.md` G-6 addition (6 audit-surfaced sub-items), `reports/phase6_1_close_out.md` (NEW), softened verdict-leak phrasing per audit C1.**
**Why:** QA/A47 cgpro verdict + ADR-63 Phase 6.2 audit identified that the canonical project-status surface and the chain's external-facing summary need to INCORPORATE the audit's 5 convergent (3/3) + 5 sharp single-provider critiques. Per cgpro QA/A47 next_action: "After the review is archived, do a separate methodology-consolidation commit updating docs/project_status.md, BACKLOG.md, and a close-out summary with the critique incorporated." Per ADR-63 5-step action list: (1) soften verdict-leak phrasing in project_status.md + close-out, (2) document seed_157 reclassification trajectory honestly, (3) tighten freeze-rule carve-out definition, (4) acknowledge LLM-replay-audit gap, (5) reframe cycle verdict more honestly than "partial generalisation".

**Decision:**

* **`docs/project_status.md` rewrite (was 2026-04-28; predates the entire 6.1' chain).** New version dated 2026-04-29. Major changes:
  * **Lane status table updated** — 4 lanes still active; manual data acquisition lane row updated to list 3 manual scripts (build_calibration_seed_index + llm_author_replays + clone_target_at_sha) instead of just the indexer.
  * **§1 "Usable now"** — added bullets for `oida-code prepare-gateway-bundle` (Phase 6.1'b), the 46-inclusion calibration_seed corpus, and the 3 manual-lane scripts.
  * **§4 NEW** — "Phase 6.1' chain — what shipped + what the audit said". Lists 8 sub-blocks honestly. Frames the chain as "shipped its discipline; produced 2 claim-supporting round-trip outcomes (1 train + 1 holdout) plus 1 honest negative; the empirical signal is thin (N_pinned=5)". Avoids "FIRST holdout success" / "partial generalisation" headline framing per audit C1.
  * **§5 roadmap updated** — Phase 6.1' marked CLOSED; Phase 6.2 marked CLOSED; corpus-quality maintenance listed as deferred.
  * **§8 NEW: "Phase 6.2 audit-informed caveats (read this if you read nothing else in §4-7)"** — 6 caveats one per audit theme: hard-wall-strict-letter-intact-but-spirit-wobbly; seed_157 reclassification trajectory; freeze-rule carve-out should be operationally bounded; LLM-replay-audit gap; N=5 statistical thinness; ADR-56 spirit-tension on seed_065. This section is the canonical reading of the chain's net standing.
  * **§9 cross-references** updated to include the audit's aggregate + per-provider critiques.

* **`BACKLOG.md` G-6 addition** — 6 sub-items (G-6a..f) capturing the audit's unresolved discipline-spirit gaps:
  * G-6a — LLM-replay-audit gap (Pydantic checks shape, not content; replay could be hallucinated).
  * G-6b — Freeze-rule "predeclared env bootstrap" carve-out should be operationally bounded (only flags pre-existing the pass, not added in response to failures).
  * G-6c — Seed authoring quality is the unguarded human step (3/46 pinned; 43 unpinned likely have similar defects).
  * G-6d — N=5 statistically thin for any generalisation claim; ratio guard is brittle at this size.
  * G-6e — ADR-56 spirit-tension on seed_065: bootstrap fixes motivated by holdout's own failure.
  * G-6f — seed_157 demotion-and-replace deferred to corpus-quality maintenance, NOT Phase 6.1' rewriting.
  All 6 are flagged as "documented; none scheduled; re-enter scope only when the operator names a specific phase that addresses them with an in-scoping ADR".

* **`reports/phase6_1_close_out.md` (NEW)** — single-page chain close-out summary that combines the 8 sub-block deliverables, the audit findings, the operator-authored cycle verdict (audit-informed), and what the chain explicitly does NOT claim. The audit-informed cycle verdict (final form):
  * "Phase 6.1' shipped corpus discipline + bundle generator + holdout discipline + bootstrap fixes; produced 2 claim-supporting round-trip outcomes (1 train + 1 holdout) and 1 honest negative. The chain validates the discipline end-to-end. It does NOT demonstrate broad cross-target generalisation; the empirical signal is thin (N_pinned=5) and the Phase 6.2 audit identified 6 unresolved discipline-spirit gaps (G-6a..f in BACKLOG.md)."
  * This supersedes the QA/A47 verdict_q3 sentence in the canonical reading. The QA/A47 sentence is preserved verbatim in `QA/A47.md` for record but the project-status / close-out reading is the audit-informed version.

* **No deletions of historical reports.** The Phase 6.1'a..h reports retain their original phrasing ("FIRST holdout generalisation success", "partial holdout generalisation", "first claim-supporting outcome") — they are historical record. The audit's softened framing only propagates to NEW canonical surfaces (`docs/project_status.md`, `reports/phase6_1_close_out.md`, the Phase 6.2 audit aggregate). External readers should consult these canonical surfaces, NOT the Phase 6.1'h headline.

* **No retroactive renumbering.** ADR-55 stays retracted by ADR-57; ADR-58..63 stay as shipped; this commit adds ADR-64 (consolidation). Memory-bank/decisionLog.md preserves the trajectory rather than rewriting it.

* **No modification of generator / verifier / clone helper / seed partitions / holdout scopes.** Per cgpro QA/A47 hard rule applied to BOTH the audit commit (Phase 6.2 / `101e633`) AND this consolidation commit. Documentation-only.

**Accepted:**
* The audit-informed framing in `docs/project_status.md` §8 IS the canonical reading. External summaries should use this language, not the Phase 6.1'h headline.
* The 6 G-6 backlog items capture the audit's unresolved discipline-spirit gaps. None are scheduled; they re-enter scope only when an explicit ADR proposes them as in-scope for a named phase.
* Per cgpro QA/A47: corpus-quality maintenance (specifically seed_157 demote-and-replace) is a SEPARATE LATER labelled task, NOT done in this consolidation commit.
* The cycle verdict reframes from "partial holdout generalisation" (Phase 6.1'h headline + QA/A47 verdict_q3) to "discipline-validated, empirical signal thin, 6 unresolved gaps" (close-out + project_status §8). The reframing is the consolidation's load-bearing ouput.
* Historical reports retain their original phrasing for traceability of the trajectory.

**Rejected:**
* Retroactively editing the Phase 6.1'h or Phase 6.1'e step 1-3 reports to soften their headlines — would erase the trajectory the audit identified. The reports stay as shipped; the consolidation supersedes them in the canonical reading.
* Demoting seed_157 in this commit — per cgpro QA/A47 corpus-quality maintenance is SEPARATE.
* Adding tests for the consolidation — there is no functional code change to test.
* Modifying the runtime path (`src/oida_code/`) or the verifier or the clone helper — per cgpro QA/A47, this commit is documentation-only.
* Renaming or removing the historical phase reports — they record the trajectory.
* Adding a 7th BACKLOG item for "the chain as a whole was over-confident" — too meta; the 6 sub-items capture the specific gaps cleanly.
* Pivoting to AI-tier rerun on a fresh surface OR seed_157 demote-and-replace IN this commit — per cgpro QA/A47 next_action, both are explicitly deferred.

**Outcome:** Methodology consolidation lands as 1 commit (this) touching: `memory-bank/decisionLog.md` (this ADR), `memory-bank/progress.md` (timeline), `docs/project_status.md` (rewrite from 2026-04-28 stale → 2026-04-29 audit-informed; new §4 + §8), `BACKLOG.md` (G-6a..f added between G-5 and "What this file is NOT"), `reports/phase6_1_close_out.md` (NEW). Test count UNCHANGED at 1128. ZERO new dependency. ZERO MCP runtime. ZERO provider tool-calling. ZERO change to `enable-tool-gateway` default. ZERO change to runtime path code (this commit edits docs/ + reports/ + BACKLOG.md + memory-bank/ only). Manual-lane scripts unchanged at 3. The Phase 4.7 + 5.0 + 5.1 + 5.2 + 5.3 + 5.4 + 5.5 + 5.6 + 5.7 + 5.8 + 5.8.x + 5.9 + 6.0 + 6.0.x + 6.0.y + 6.0.y' + 6.0.z + 6.1'a-pre + 6.1'a + 6.1'b + 6.1'c + 6.1'd + 6.1'e (steps 1-4) + 6.1'f + 6.1'g + 6.1'h + 6.2 anti-MCP / no-product-verdict / lane-separation / partition-discipline / holdout-discipline / freeze-rule / audit-as-block locks remain ACTIVE. Phase 6.1' chain is now formally CLOSED with the audit-informed reading as canonical. Future work options (per cgpro QA/A47, ADR-63, this ADR): corpus-quality maintenance (G-6f); Phase 7 research moat (off critical path per project-rule 2); G-1 official fields (still blocked); G-3 large-scale validation (still missing); operator-named other priorities. NO commit to which option will be pursued or when.

[2026-04-29 15:30:00] - **ADR-65: Phase 6.1' corpus-quality maintenance v1 — seed_157 demoted to train; seed_018 (attrs#1529) pinned as new holdout with audit-informed Tier-3 (narrow test_scope per G-6c); freeze-rule pass produces 2nd holdout `verification_candidate`.**
**Why:** Phase 6.2 audit (ADR-63) + ADR-64 close-out left seed_157 in a "honest negative" state with documented seed-record authoring quality issue (over-broad operator-authored test_scope). cgpro QA/A47 verdict_q1 declared "demote-and-replace" was a SEPARATE LATER labelled "corpus-quality maintenance" task, NOT part of Phase 6.1' rewriting. ADR-65 ships that maintenance task as v1.

**Decision:**
* **seed_157 demoted to train** with `partition_pinned_at: 2026-04-29T15:00:00Z` (matching the demotion timestamp). The candidate_reason field appends a documented demotion note explaining: original 6.1'c rationale preserved + Phase 6.1'h freeze-rule pass result + audit identification of over-broad test_scope + cgpro QA/A47 demote-and-replace protocol cited. The original Tier-3 fields are NOT cleared — they remain the historical record of the holdout's authoring (per audit lesson: don't erase the trajectory).
* **First candidate considered: seed_058_pallets_itsdangerous_378** (FIPS+SHA-1 lazy_sha1 fix, pure-Python). REJECTED on packaging-style discovery: itsdangerous declares test deps in `requirements/tests.txt` (pip-tools / older pattern), NOT in `[project.optional-dependencies]` (PEP 621) or `[dependency-groups]` (PEP 735). Adding a 3rd `--install-requirements-file` flag to the clone helper would be exactly the carve-out widening audit G-6b warned against. Reverted seed_058 to its initial unpinned state.
* **Selected candidate: seed_018_python_attrs_attrs_1529** ("Add instance support to attrs.fields()", `capability_sufficient`). Selected for: pure-Python diff (no rust-core or build-backend complications); ONE existing test modified (`tests/test_make.py::TestFields::test_instance` — was raising TypeError, now asserts `fields(C()) is fields(C)`); ONE new sibling test added (`test_handler_non_attrs_instance` for the negative path); PEP 735 `[dependency-groups] tests` (already supported by the clone helper's `--install-group` flag, NO carve-out widening); 4th distinct repo in the pinned set (`python-attrs/attrs` joins pytest-dev/pytest, simonw/sqlite-utils, hynek/structlog).
* **Tier-3 fields authored AUDIT-INFORMED (G-6c lesson):**
  * `claim_id: "C.attrs_fields_instance_support.capability_sufficient"`.
  * `claim_type: "capability_sufficient"`.
  * `claim_text` — paragraph naming the change in `_make.py:fields()` + the new instance-handling branch + the updated TypeError message.
  * `evidence_items` — 2 items: source=git for the diff shape; source=ticket for the test rewrites (test_instance modified + test_handler_non_attrs_instance NEW).
  * **`test_scope: "tests/test_make.py::TestFields::test_instance"`** — NARROW test_scope per audit G-6c: specific test name, NOT class scope. The chosen test is the modified one that DEMONSTRATES the new capability (`fields(C()) is fields(C)`).
  * `expected_grounding_outcome: "evidence_present"`.
  * `label_source: "yann_manual_review"`.
  * `human_review_required: false`.
  * `partition: holdout`, `partition_pinned_at: 2026-04-29T15:30:00Z`.
* **Freeze-rule pass executed at code SHA `e742867`** (post-consolidation, pre-this-commit working state). Frozen manifest: DeepSeek `deepseek-chat` (no prompt edits); no in-pass edits to generator/verifier/clone-helper/LLM-prompts/replay-shapes; predeclared env-bootstrap allowed (`--install-oida-code`, `--install-group tests`, `--import-smoke attr` — all flags pre-existing the pass per audit G-6b). seed_008 / seed_065 / seed_157 NOT re-run.
* **Frozen invocation:** `clone_target_at_sha.py --repo python-attrs/attrs --head-sha b13a7056c5f407abbd9f3e572ee48ad65afce91c --manual-egress-ok --install-oida-code --install-group tests --import-smoke attr` → `prepare-gateway-bundle` → `llm_author_replays.py` (DeepSeek 7.2s) → `verify-grounded` with PATH-prepended clone-venv pytest.
* **Outcome — seed_018 `verification_candidate` ✅:** `accepted_claims=['C.attrs_fields_instance_support.capability_sufficient']`, `unsupported=[]`, `blockers=[]`, `warnings=[]`. pytest tool result: `status=ok`, `pytest_summary_line="1 passed, 1 warning in 1.57s"`. **Second holdout-target `verification_candidate` outcome in the calibration_seed lane.** The 1-warning is benign (likely hypothesis health-check or pytest-asyncio config) and does NOT affect the verifier's claim acceptance.
* **Audit findings addressed by this commit:**
  * **G-6c (seed authoring quality):** the new holdout's test_scope is narrow (specific test, not class). The Tier-3 authoring is more careful than seed_157's (which was the audit's exhibit case for G-6c).
  * **G-6f (seed_157 demote-and-replace):** seed_157 demoted to train; new holdout pinned. G-6f is now CLOSED for v1.
  * **G-6e (ADR-56 spirit-tension on seed_065):** PARTIALLY addressed — seed_018 was pinned AFTER all the bootstrap fixes (6.1'f / 6.1'g) shipped; the freeze-rule pass succeeded at the post-fix SHA without any new tooling changes motivated by seed_018's behaviour. Unlike seed_065, seed_018's success is causally INDEPENDENT of the bootstrap fixes (the fixes were already in place before the holdout was pinned).
* **Audit findings NOT addressed by this commit (still in BACKLOG G-6):**
  ...
  * G-6b (freeze-rule carve-out should be operationally bounded): commit 2 (this same operator session) addresses this with a structural test.
  * G-6d (N=5 statistical thinness): N_pinned now 6, ratio 0.33; still thin.

**Accepted:**
* The audit-informed Tier-3 authoring (narrow test_scope, careful evidence_items) demonstrates that the lesson from seed_157 is incorporated. seed_018's clean `verification_candidate` provides empirical evidence that audit lessons reduce the seed-record authoring defect class.
* G-6f is CLOSED for v1; corpus-quality maintenance is now an active discipline.
* The first-candidate seed_058 rejection (itsdangerous's requirements/*.txt pattern) was honest engineering — adding a 3rd `--install-requirements-file` flag would have been exactly the carve-out widening G-6b warned against. Documenting the rejection in the ADR + the candidate_reason is more valuable than silently swapping.
* Holdout count is now 2/6 = 0.33 ratio, comfortably inside [0.20, 0.40] enforcing band (more comfortable than the prior 0.40 boundary).
* seed_157's original Tier-3 fields are PRESERVED in its record (along with the demotion note) — the trajectory is documented, not erased.
* DeepSeek round-trip cost: ~$0.001. Cumulative chain spend now ~$0.013.

**Rejected:**
* Adding a 3rd `--install-requirements-file PATH` flag to the clone helper to handle pallets/itsdangerous-style requirements/*.txt — direct violation of audit G-6b. Documenting the limitation in this ADR (and surfacing it as a future BACKLOG item) is the honest path.
* Clearing seed_157's Tier-3 fields when demoting — would erase the trajectory the audit identified. The fields stay; the demotion note is appended to candidate_reason.
* Re-running seed_065 / seed_157 / seed_008 in this freeze-rule pass — none are part of corpus-quality v1 (seed_065 is the existing holdout success; seed_157 is being demoted; seed_008 is the train control). Per cgpro QA/A47: only the new holdout (seed_018) gets a freeze-rule pass.
* Pinning more than one new holdout in this commit — minimal corpus-quality block. Future v2/v3 maintenance commits may pin additional holdouts as the audit-informed authoring discipline matures.
* Re-doing the entire 6.1'h freeze-rule pass at a fresh SHA — the existing 6.1'h evidence stays as historical record. Only the NEW holdout (seed_018) gets a new pass.
* Editing the pytest adapter or verifier to handle seed_157-style class-scoped failures more gracefully — that would be tooling tuning in response to a holdout failure (G-6e violation). The seed_157 demotion is the right corrective.

**Outcome:** Phase 6.1' corpus-quality maintenance v1 lands as 1 commit (this) touching: `memory-bank/decisionLog.md` (this ADR), `memory-bank/progress.md` (timeline), `reports/calibration_seed/index.json` (seed_157 demoted to train; seed_018 pinned as holdout with full Tier-3; seed_058 reverted to unpinned state — it had been speculatively edited in an earlier draft of this script then rolled back when the requirements/*.txt issue was discovered), `reports/phase6_1_corpus_quality_v1/round_trip_outputs/seed_018_python_attrs_attrs_1529/` (NEW, 9 evidence files including the verification_candidate `grounded_report.json`), `reports/phase6_1_corpus_quality_v1.md` (NEW, phase report). Test count UNCHANGED at 1128 (corpus-quality is documentation+data, no new tests). ZERO new dependency. ZERO MCP runtime. ZERO provider tool-calling in runtime path. ZERO change to runtime path code. ZERO change to `enable-tool-gateway` default. ZERO change to clone helper / generator / verifier per audit G-6b carve-out discipline. Manual-lane scripts unchanged at 3.

**Holdout discipline state at end of corpus-quality v1:**
* Pinned: 6 cases (4 train + 2 holdout) — N_pinned 5→6.
* Train: seed_008 (verification_candidate Phase 6.1'e step 3), seed_062 (Tier-3-pinned, never round-tripped), seed_142 (Tier-3-pinned, never round-tripped), **seed_157 (demoted from holdout 2026-04-29T15:00:00Z)**.
* Holdout: seed_065 (verification_candidate Phase 6.1'h), **seed_018 (verification_candidate corpus-quality v1, this ADR)**.
* Holdout ratio 2/6 = 0.33 — comfortably inside [0.20, 0.40] enforcing band.
* Generalisation tally (post-v1): 2/2 holdouts succeeded (seed_065 + seed_018). However per the audit (G-6e), seed_065's success is causally entangled with bootstrap fixes; seed_018's success is causally INDEPENDENT (pinned AFTER all fixes shipped). Cleanly-counted holdout success is 1/1 (seed_018) plus the entangled-but-still-passing seed_065 (1/1).

**Cycle verdict UPDATE (the canonical reading after corpus-quality v1):**
...
[2026-04-30 07:30:00] - **ADR-72: G-6d.2 corpus pinning tranche - four additional AI-authored public-diff pins, post-freeze local feasibility, N=14.**
**Why:** After ADR-71 was committed and pushed, the user directed Codex to report to cgpro and continue. cgpro QA/A53 recommended another pinning-only tranche rather than replay generation, human-review relabeling, Phase 7, or official fusion-field work. The target remains N>=20; this block advances the corpus from N=10 to N=14 while preserving the no-replay and freeze-before-outcome discipline.

**Decision:**

* Pin exactly four formerly unpinned existing records:
  * `seed_037_tiangolo_typer_1695` - Typer #1695 rich traceback code line preservation, `capability_sufficient`, train.
  * `seed_075_simonw_sqlite_utils_653` - sqlite-utils #653 ON CONFLICT upsert path, `capability_sufficient`, train.
  * `seed_161_hynek_structlog_757` - structlog #757 mutable `ConsoleRenderer.columns`, `capability_sufficient`, holdout.
  * `seed_162_hynek_structlog_756` - structlog #756 mutable `ConsoleRenderer.sort_keys`, `capability_sufficient`, train.
* Freeze partitions at `2026-04-30T07:18:00Z`, before scoped pytest feasibility.
* Deterministic holdout rule: sort selected case ids, compute `sha256('g6d2-holdout:' + case_id)`, and assign the lowest hash to holdout. Result: `seed_161_hynek_structlog_757`.
* Record selection in `reports/phase6_d_2_pinning/selection.{json,md}` and post-freeze feasibility in `reports/phase6_d_2_pinning/feasibility.{json,md}`.
* Keep all four new pins as `label_source=ai_authored_public_diff_review`, `human_review_required=true`, `llm_assist_used=true`.
* Convert ADR-71 tests from live-N=10 guards into historical transition guards; add G-6d.2 tests for the new live N=14 state.

**Post-freeze feasibility:** All four local clone/scoped-pytest checks passed without new clone-helper flags or runtime changes:

* seed_037: `tests/test_tracebacks.py::test_rich_exceptions_dont_truncate_code_on_wide_terminal` -> `1 passed in 0.66s`.
* seed_075: `tests/test_upsert.py::test_upsert` -> 2 parametrized invocations passed, pytest rc=0.
* seed_161: `tests/test_dev.py::TestConsoleRenderer::test_columns_property` -> `1 passed in 0.07s`.
* seed_162: `tests/test_dev.py::TestConsoleRenderer::test_sort_keys_property` -> `1 passed in 0.07s`.

**Accepted:** Use the remaining existing public index first; no fresh GitHub harvesting. A dedicated `.tmp/g6d2_clones` directory was used via the predeclared `--clones-dir` flag to avoid collision with no-checkout diff-inspection clones. This is recorded in the feasibility artifact and did not add a clone-helper flag.

**Rejected:** replay bundles, `round_trip_outputs`, verifier pass JSON, grounded reports, provider evidence, PAT_GITHUB/GitHub API use, runtime/provider/default-gateway edits, MCP changes, clone-helper flag additions, and any product/generalisation claim. Human-review provenance is not closed autonomously.

**Outcome:** ADR-72 lands as one G-6d.2 pinning block touching `QA/A53.md`, `reports/calibration_seed/index.json`, `reports/phase6_d_2_pinning/selection.{json,md}`, `reports/phase6_d_2_pinning/feasibility.{json,md}`, `tests/test_phase6_d_1_pinning.py`, `tests/test_phase6_d_2_pinning.py`, `BACKLOG.md`, `docs/project_status.md`, `docs/calibration_seed_expansion_protocol.md`, `memory-bank/decisionLog.md`, and `memory-bank/progress.md`. Live corpus state: 46 records, 14 pinned, 10 train, 4 holdout, holdout ratio 0.29. G-6d remains OPEN toward N>=20. G-6c remains PARTIAL because the AI-authored pins still require independent human review.

The Phase 4.7 + 5.0 + 5.1 + 5.2 + 5.3 + 5.4 + 5.5 + 5.6 + 5.7 + 5.8 + 5.8.x + 5.9 + 6.0 + 6.0.x + 6.0.y + 6.0.y' + 6.0.z + 6.1'a-pre + 6.1'a + 6.1'b + 6.1'c + 6.1'd + 6.1'e (steps 1-4) + 6.1'f + 6.1'g + 6.1'h + 6.2 + consolidation v1 + corpus-quality v1 + G-6b structural pin + consolidation v2 + Phase 6.a static audit + Phase 6.a.1 manual semantic review + G-6d.0 planning + G-6d.1 pinning + G-6d.2 pinning anti-MCP / no-product-verdict / lane-separation / partition-discipline / holdout-discipline / freeze-rule / audit-as-block / corpus-quality-v1 / predeclared-bootstrap-pin locks remain ACTIVE.

[2026-04-30 09:30:00] - **ADR-73: G-6d.3 corpus pinning tranche stopped - frozen attempt hit older requirements-file dependency boundary before pytest outcome.**
**Why:** After ADR-72 was committed and pushed, the user directed autonomous continuation. cgpro QA/A54 selected a third pinning-only tranche: exactly four more existing unpinned records, split 3 train / 1 holdout, moving from N=14 to intended N=18 if feasibility passed. The same hard walls applied: no replay, no provider evidence, no fresh GitHub API/PAT path, no runtime/provider/MCP/default-gateway changes, no clone-helper flag widening, and no replacement after post-freeze failure.

**Decision and stop:** Codex inspected public diffs and froze an attempted selection at `2026-04-30T09:18:00Z`: `seed_058_pallets_itsdangerous_378` intended train, `seed_071_simonw_sqlite_utils_689` intended train, `seed_074_simonw_sqlite_utils_658` intended train, and `seed_159_hynek_structlog_759` intended holdout by `sha256('g6d3-holdout:' + case_id)`. The first post-freeze feasibility command for `seed_058` cloned and installed the target but failed at `--install-group tests` because ItsDangerous at that SHA has no PEP 735 `[dependency-groups]`. The target's `tox.ini` instead declares `deps = -r requirements/tests.txt`, and that file contains `pytest==8.1.1`. A diagnostic pytest attempt in the venv failed with `No module named pytest`; scoped pytest outcome was not reached.

**cgpro stop ruling:** Manually installing `requirements/tests.txt` would widen the current block's dependency-install boundary because `seed_058` was already documented as the older requirements-pattern risk. Therefore ADR-73 must not be committed as a successful G-6d.3 pin, must not manually rescue `seed_058`, and must not replace any selected case after freeze.
